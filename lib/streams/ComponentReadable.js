...
class ComponentReadable extends stream.Readable {
  ...
  _getComponentContext(context) {
    ...
    const params = moduleHelper.getStoreParamsFromAttributes(attributes);
    const instanceId = moduleHelper.getStoreInstanceId(storeName, params);

    componentContext.element = null;
    componentContext.name = context.currentComponent.name;
    componentContext.attributes = attributes;
    componentContext.storeParams = params;
    ...
    componentContext.sendAction =
      (name, args) => context.storeDispatcher.sendAction(instanceId, name, args);
