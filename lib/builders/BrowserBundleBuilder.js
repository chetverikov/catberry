...
class BrowserBundleBuilder {
   ...
   * @return {Promise} The promise for finished work.
   */
  build() {
   ...
   * @return {Browserify} The browserify instance.
   * @private
   */
  _createAppBundler() {
    if (this._appBundler) {
   ...
   * @return {Browserify} The browserify instances.
   * @private
   */
  ...
  _setTransformations() {
    ...
    let currentIndex = this._browserifyTransformations.length - 1;
   ...
   */
  _setPlugins() {
    let currentIndex = this._browserifyPlugins.length - 1;
