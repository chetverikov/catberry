    ...
    if (this.attributes.length) {
      ...
      while (attributeIndex++ < this.attributes.length) {
        ...
        if (attribute) {
          ...
          if (countOfBreaks) {
            attributeValues = values.slice(valueIndex, valueIndex + countOfBreaks);
            valueIndex += countOfBreaks;
          }

          if (attributeValues !== null && !attribute.isSkipped(attributeValues)) {
            attributes.push(attribute.compile(attributeValues));
          }
        }
      }
    }
