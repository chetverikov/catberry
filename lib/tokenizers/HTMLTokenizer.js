...
class HTMLTagTokenizer {
  ...
  /**
   * Sets html string to tokenize.
   * @param {string} tagHTML Tag HTML string.
   */
  setHTMLString(tagHTML) {
  ...
  initial() {
    ...
    const current = this._source[this._currentIndex];
    const next = this._source[this._currentIndex + 1];
    ...
    if (current === '<') {
      ...
      this._currentState = STATES.TAG_OPEN;
      return;
    }
  ...
  content() {
    ...
    while (this._currentIndex < this._source.length) {
      const next = this._source[this._currentIndex + 1];
      if (this._source[this._currentIndex] === '<' && next !== '/' && next !== '!') {
        this._currentState = STATES.NO;
        return;
      }
      this._currentIndex++;
  ...
  selfClosingStartTagState() {
    ...
    if (this._source.length <= this._currentIndex) {
      this._currentState = STATES.END;
      return;
    }
