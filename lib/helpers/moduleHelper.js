...
const helper = {
  ...
  getStoreInstanceId(storeName, params = {}) {
    const stringOfKeys = Object.keys(params).sort().join('');
    const stringOfParams = Object.values(params).map(String).sort().join('');

    return genHashByString(`${storeName}${stringOfKeys}${stringOfParams}`);
  },
};

function genHashByString(string) {
  let h = 0xdeadbeef;
