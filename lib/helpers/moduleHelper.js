...
const helper = {
	...
	getStoreCacheKey(storeName, storeInstanceId) {
		const instance = storeInstanceId ? `[${storeInstanceId}]` : '';

		return `${storeName}${instance}`;
	},

	/**
	 * Return true if a passed store name contain dynamic word
	 *
	 * @param {string} storeName Store name
	 * @returns {boolean} True if name contain "dynamic"
	 */
	isDynamicStore(storeName) {
		return helper.STORE_DYNAMIC_NAME_REGEXP.test(storeName);
	},

	/**
	 * Create object of parameters for store from component attributes
