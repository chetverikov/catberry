...
class StoreDispatcher {
	...
	getStoreData(storeName, storeParams) {
		...
		if (storeName in this._lastData) {
			...
			if (!storeParams && existTime <= this._lastData[storeName].lifetime) {
	 ...
	 * @param {string} storeName Name of the store.
	 * @param {Object?} storeParams Params of the store.
	 * @returns {Object} Store context.
	 * @private
	 */
	_getStoreContext(storeName, storeParams) {
		const storeContext = Object.create(this._currentBasicContext);
		propertyHelper.defineReadOnly(storeContext, 'name', storeName);
		propertyHelper.defineReadOnly(storeContext, 'params', storeParams);
	 ...
	 */
	getStore(storeName, storeParams) {
		if (!storeName) {
			return null;
		}
		...
		StoreConstructor.prototype.$context = this._getStoreContext(storeName, storeParams);
		this._storeInstances[storeName] = new StoreConstructor(this._serviceLocator);
		this._storeInstances[storeName].$context = StoreConstructor.prototype.$context;
