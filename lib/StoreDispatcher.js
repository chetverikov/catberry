...
class StoreDispatcher {
	 ...
	 * @param {String?} options.instanceId Instance Id of store
			...
			Object.keys(this._storeInstances)
				.forEach(storeName => {
					Object.keys(this._storeInstances[storeName])
						.forEach(instanceId => {
							const params = this._storeInstances[storeName][instanceId].$context.params;
							this._storeInstances[storeName][instanceId].$context = this._getStoreContext(storeName, {
								instanceId, params
							});
						})
				});
		...
		Object.keys(changed)
			.forEach(storeName => {
				Object.keys(this._storeInstances[storeName])
					.forEach(instanceId => {
						const params = this._storeInstances[storeName][instanceId].$context.params;
						const store = this.getStore(storeName, {instanceId, params});
