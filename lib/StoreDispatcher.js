...
class StoreDispatcher {
   ...
   */
  sendActionToAllInstances(storeName, actionName, args) {
    if (!this._lastState) {
      return this._errorState();
    }

    let instancesData = this._getStoreInstancesDataByName(storeName);

    if (!Array.isArray(instancesData) || !instancesData.length) {
      const store = this.getStore(storeName);

      if (!store) {
        return this._errorStoreInstancesNotFound(storeName);
      }

      instancesData = this._getStoreInstancesDataByName(storeName);
    }

    const promises = instancesData.map((instanceData) => {
      const handleMethod = moduleHelper.getMethodToInvoke(instanceData.instance, 'handle', actionName);

      return moduleHelper.getSafePromise(() => handleMethod(args))
        .then((result) => {
          const actionDetails = {
            instanceId: instanceData.instance.$context.instanceId,
            storeName,
            actionName,
            args,
          };
   ...
   *
   * @return {Array<string>} Instances of stores that have been changed.
   */
  setState(parameters, basicContext) {
    ...
    const changedStoreInstances = Object.create(null);

    Object
      .keys(changed)
      .forEach((storeName) => {
        const instancesData = this._getStoreInstancesDataByName(storeName);
        let hasInstanceWithEmptyParams = false;
        let instanceData = null;

        while (instanceData = instancesData.pop()) {
          if (instanceData && instanceData.instance) {
            if (!Object.keys(instanceData.params).length) {
              hasInstanceWithEmptyParams = true;
            }

            instanceData.instance.$context
              .changed()
              .forEach((storeInstance) => {
                changedStoreInstances[storeInstance] = true;
              });
          }
        }

        if (!hasInstanceWithEmptyParams) {
          const store = this.getStore(storeName);
          if (store) {
            store.$context
              .changed()
    ...
    return Object.keys(changedStoreInstances);
  }
  ...
  _getStoreContext(storeName, params = {}) {
    ...
    propertyHelper.defineReadOnly(storeContext, 'instanceId', storeInstanceId);
    ...
    storeContext.getStoreData = (sourceStoreName, params = {}) => sourceStoreName === storeName ?
      Promise.resolve(null) :
      this.getStoreData(sourceStoreName, params);

    storeContext.setDependency = (dependName, params = {}) => {
      const dependInstanceId = moduleHelper.getStoreInstanceId(dependName, params);

      if (!(dependInstanceId in this._dependants)) {
        this._dependants[dependInstanceId] = Object.create(null);
      }

      this._dependants[dependInstanceId][storeInstanceId] = true;
    };
    storeContext.unsetDependency = (dependName, params = {}) => {
      const dependInstanceId = moduleHelper.getStoreInstanceId(dependName, params);

      if (dependInstanceId in this._dependants) {
        delete this._dependants[dependInstanceId][storeInstanceId];
      }
    };
    storeContext.sendAction = (storeName, name, args) => this.sendActionToAllInstances(storeName, name, args);

    return storeContext;
  }

  /**
   * Return store name for specified instance id
   *
   * @param {String} storeInstanceId Store instance id
   * @return {String|null} Store name
   */
  getStoreNameByInstanceId(storeInstanceId) {
    const storeData = this._getStoreInstanceDataById(storeInstanceId);

    return storeData && storeData.storeName ? storeData.storeName : null;
  }
   ...
   * @return {Promise<Error>} The promise for the error.
   */
  _errorStoreInstancesNotFound(name) {
    return Promise.reject(new Error(`Instances of store "${name}" not found`));
  }
